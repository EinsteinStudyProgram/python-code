...
class AttenceExcel:
    def judge(self,department):
        ...
        satelliteTV_department = ["北京事业部","陈汝涵团队","陈歆宇团队","单丹霞团队","导摄部","都艳团队","服装化妆部","覆盖部","规划编排部","后期制作团队",
                                  "节目制作中心","节目资源部","孔晓一团队","李哲团队","梁书源团队","刘建立团队","刘蕾团队","刘庆荣团队","罗强良团队","罗昕团队",
        ...
        while start < rows:
            ...
            while True:
                ...
                if end+1>rows:
                    ...
                    sheet1.write_merge(0, 0, 0, 6, department,style)
                    sheet1.write_merge(1, 1, 0, 6, '2017年1月工作日：18天')
                    ...
                    sheet1.write(2, 4, '来台天数')
                    #sheet1.write(2, 4, '？表示低于暂定的考勤值')
                    # 异常所属单元格的格宽度
                    #sheet1.col(4).width = 6000
                    sheet1.write(2, 5, '核查后反馈信息')
                    # 设置单元格宽度
                    sheet1.col(5).width = 6666
                    sheet1.write(2,6,'本人签名')
                    sheet1.col(6).width = 4888
...
OP = AttenceExcel()
OP.operator(r'E:\正式环境1月考勤记录.xls')
